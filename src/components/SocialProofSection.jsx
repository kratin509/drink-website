...
function Card({ d }) {
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave}
      ...
      style={{ padding: '20px 22px 16px', border: '1px solid rgba(255,255,255,0.07)', transformStyle: 'preserve-3d' }}>
      <div className="absolute top-0 right-0 w-5 h-5"
        style={{ background: d.a, clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }} />
      <div className="flex justify-between items-center mb-4">
        <span style={{ fontSize: '1.7rem', lineHeight: 1 }}>{d.e}</span>
        <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '0.56rem', letterSpacing: '0.18em', textTransform: 'uppercase', padding: '2px 6px', background: `rgba(${d.r},.12)`, color: d.a }}>{d.p}</span>
      </div>
      <blockquote className="flex-1 mb-4 leading-relaxed" style={{ fontFamily: "'Barlow', sans-serif", fontSize: '0.83rem', color: '#1a1a1a' }}>
        "{d.q}"
      </blockquote>
      <div className="flex justify-between items-center pt-3" style={{ borderTop: '1px solid rgba(0,0,0,0.08)' }}>
        <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '0.8rem', color: d.a }}>{d.h}</span>
        <span style={{ fontSize: '0.68rem', opacity: 0.34 }}>{d.f}</span>
      </div>
    </div>
  )
}
    ...
    <section ref={sectionRef} className="section-frame" style={{ background: 'var(--dark)' }}>
      ...
      <div className="absolute inset-0 z-20 flex flex-col px-8 md:px-16 lg:px-24 pt-10 pb-8">
        ...
        <div ref={headRef} className="mb-5">
          ...
          <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: '0.93rem', color: 'rgba(255,255,255,0.36)', maxWidth: '36ch' }}>
